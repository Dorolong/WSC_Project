...
## 4-1. 검증 설계 — 무엇으로 "잘됐다"고 판정할까

만들기 전에 판정 기준을 정해둔다. 나중에 정하면 **결과에 맞춰 기준이
움직이고**, 자기 채점으로 흘러간다.

### 함정 ① 자기 채점

같은 날씨로 튜닝하고 채점하면 시험 문제로 공부하고 그 문제로 시험 보는 것이다.
그 날씨에만 맞춰진 해가 나와도 알 방법이 없다.

| | 용도 | 시드 |
|---|---|---|
| **탐색용** | 목적함수 평가(표 생성 시 평균낼 날씨) | `WEATHER_SEEDS_TRAIN` = 1~5 |
| **검증용** | 만든 것을 채점 | `WEATHER_SEEDS_VALID` = 101~130 |

`evaluate_policy()`가 둘이 겹치면 **예외를 던져 막는다.**

> 참고: 이 분리 전까지 Optuna는 시드 1~5로 탐색하고 같은 1~5로 채점했다.
> 그때 나온 파라미터는 in-sample 결과다.

### 함정 ② 정상 출발 한 번으로는 표의 0.01%만 본다

`SOC=1.0` 정시 출발로 쭉 달리면 **경로 하나만** 지나간다.
표가 600만 칸인데 한 번의 주행이 방문하는 건 600칸 남짓이다.
그런데 **정책의 존재 이유가 계획에서 벗어난 상태에 답하는 것**이다.

→ 일부러 나쁜 상태로 출발시킨다 (`START_STATES`):
정상 / SOC 70% / SOC 50% / 2시간 지연 / SOC70%+2시간 지연

> 한계: 지금은 **출발 상태**만 바꿀 수 있다. 주행 중간에 교란을 주입하려면
> `run_simulation()` 수정이 필요하다(물리 파일이라 사용자 영역).

### 무엇을 보나 — 완주 여부만으로는 부족

| 지표 | 왜 |
|---|---|
| **완주율** | 검증 날씨 중 몇 %에서 완주하나 |
| **평균속도** | 완주만 하고 느리면 의미 없다 (완주한 것만 집계) |
| **최저 SOC** | 완주해도 여유가 없었으면 운이 좋았던 것 |
| **도달률** | 실패한 경우 얼마나 아깝게 실패했나 |

### 기준선 대비로만 "좋아졌다"를 말할 수 있다

```python
from scripts.main import build_objective, compare_policies
obj, ctx = build_objective()
compare_policies({"규칙기반(기준선)": mpc_default_params,
                  "DP 표":          dp_params}, ctx)
```

**`mpc/mpc_controller.py`의 LV1~LV8을 지우지 말라는 이유가 이것이다.**
비교 대상이 없으면 나아졌는지 말할 수 없다.

### 실행 비용

전 구간 시뮬레이션 1회가 **약 45초**다. 시드 30 × 출발상태 5 = 150회면
**약 2시간**이다. 빠른 확인에는 `seeds`/`start_states`를 줄여 쓰고,
전체 검증은 배치로 돌린다.

### 이건 별개다

위 항목은 전부 **"모델이 맞다는 전제 아래" 정책이 좋은가**를 본다.
모델 자체가 맞는지는 **실측 대조(L3)로만** 알 수 있다.
격자를 촘촘히 하고 로버스트니스를 검증해도, `Cd`가 틀렸으면
**틀린 세계에서 최적인 정책**이다. 두 검증을 섞지 말 것.

### 격자는 얼마나 촘촘해야 하나

DP에는 trial 개념이 없다(한 번 돌면 끝, 항상 같은 답).
대신 **격자 해상도**가 손잡이인데 **수확체감이 있다.**
장난감 DP 기준 SOC격자 11→21에서 9분 개선, 그 뒤 30배를 키워도 0.5분 미만이었다.
→ **성기게 잡고 빠르게 반복하다가, 물리가 실측으로 굳은 뒤 키운다.**
격자 4배보다 `Cd` 실측 한 번이 정확도를 훨씬 올린다.
